                ...
                <tbody>
                    @foreach($news as $new)
                        <tr>
                            ...
                            <td>
                                @if ($new->image)
                                    <img src="{{ asset('assets/img/news/' . $new->image) }}" alt="News Image" style="max-width: 3cm;">
                                @else
                                    No Image
                                @endif
                            </td>
                            <td>{{ $new->created_at }}</td>
                            <td class="flex items-center pt-8">
                                <a href="/admin/news/edit/{{ $new->id }}" class="btn btn-primary btn-sm mx-3">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                                <form method="post" action="/admin/news/delete/{{ $new->id }}" class="flex">
                                    @csrf
                                    @method('DELETE')
